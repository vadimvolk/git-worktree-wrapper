...
The algorithm ``gww clean`` runs to resolve a provider for the current source: test the origin host against each user-declared ``providers.<kind>.host_patterns`` (a list of regex strings) in config order; first match wins. **No ``GWW_PROVIDER`` environment override. No built-in defaults** for hosted instances — users on ``github.com`` / ``gitlab.com`` / ``codeberg.org`` / etc. must declare the provider in their config, or rely on the ``--merged`` git fallback. Failing all declared patterns means "no provider resolvable" and the ``--merged`` filter falls back to ``git branch --merged <default>`` — emitting a stderr warning when providers *are* declared but none match the host. The per-kind modules in ``src/gww/providers/`` (``github.py`` / ``gitlab.py`` / ``gitea.py``) are reference starting points that document a sensible default ``host_patterns`` and ``merged`` template; they are not auto-applied.
