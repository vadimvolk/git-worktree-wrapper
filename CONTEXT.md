...
**`--merged` filter**:
The default filter for ``gww clean``. Marks as cleanable (see *Cleanable worktree*, i.e. eligible for removal) only worktrees whose branch has an upstream merge request in the ``merged`` state — equivalently, ``--merged`` **removes** merged-MR worktrees and **keeps** the rest. When no provider is configured and the origin host doesn't auto-resolve to one, falls back to git's "fully merged into default branch" check (``git branch --merged <default>``); gww emits a stderr warning on this fallback except when there is no origin remote or no providers are declared (the deliberate-fallback case, which stays silent). The user-visible flag stays ``--merged`` in both cases (see ADR-0015). A worktree whose branch has no MR at all (e.g. ``wip/foo``) is not cleanable and stays in the source: silent keep is the contract, regardless of fallback path.
...
The interactive prompt ``Filter: --merged (provider: github). Delete matching worktrees? [y/N]`` issued once per ``gww clean`` run after the plan is printed and before any deletion. The ``--merged`` token becomes ``--all`` when that filter is active; the ``(provider: <name>)`` suffix appears only when ``--merged`` is the active filter AND a provider resolved, otherwise the prompt is just ``Filter: --merged. Delete matching worktrees? [y/N]``. Triggered on stdin EOF or non-``y`` answer: the command exits 0 having printed the plan but with no side effects. Skipped by ``--yes``/``-y``.
...
**Provider**:
A named filter-check rule under ``providers:``, selected by a ``when`` predicate, whose ``filter`` command template ``gww clean`` renders per branch to decide whether a branch is cleanable. The name is **free-form** — a provider is not intrinsically a forge kind. ``gww clean`` never makes direct API calls: the ``filter`` template shells out to whatever CLI the user configured (``gh`` / ``glab`` / ``tea`` / a ``jq`` pipeline / anything). The ``gww init config`` template ships a commented ``providers:`` block with worked GitHub / GitLab / Gitea examples users copy in; these are the only shipped examples — there are no per-kind reference modules in the codebase.
_Avoid_: forge (gitlab-specific term that leaked into the wider world), remote, host (a host is a hostname, not a provider), kind (dropped 2026-07-15 per ADR-0021 — providers are named, not typed).

**Provider selection** (rewritten 2026-07-15, per ADR-0021; supersedes *Origin-based provider detection*):
The algorithm ``gww clean`` runs to resolve a provider for the current source: evaluate each user-declared provider's ``when`` predicate against the source's URI+tag context in config order; first match wins — identical to how ``sources:`` rules select. The ``when`` sees the URI functions (``host()``, ``port()``, ``protocol()``, ``path(n)``, ``uri()``), tags (``tag()``, ``tag_exist()``), but **not** ``branch()`` (the branch isn't known until ``filter`` runs per branch). **No ``GWW_PROVIDER`` environment override. No built-in defaults** — users must declare the provider in their config, or rely on the ``--merged`` git fallback. No provider matching (or no origin URI) means "no provider resolvable" and the ``--merged`` filter falls back to ``git branch --merged <default>`` — emitting a stderr warning when providers *are* declared but none match.
_Avoid_: provider autodiscovery, magic detection, host_patterns (the pre-0021 regex mechanism, removed).

**Provider filter command**:
The single string ``providers.<name>.filter`` evaluated by gww's existing template engine (``gww.template``) with the same functions available as path templates and ``command`` actions: ``branch()``, ``host()``, ``path(n)``, ``protocol()``, ``uri()``, ``port()``, ``tag(name, default)``, ``tag_exist(name)``, and project-context functions like ``current_worktree()``. The template is resolved against a per-worktree ``TemplateContext`` so commands like ``gh pr list --head branch()`` see the right branch for each worktree. The rendered command is invoked via the shell; ``gww clean`` reads only its **exit code** — no stdout parsing, no JSON, no canonical enum. **Exit 0 means "the branch is cleanable" (e.g., an MR/PR for this branch is in the merged state), the worktree will be removed.** Non-zero means "do not remove" (the worktree stays) — whether that's "no MR exists", "MR exists but isn't merged", "auth failed", "rate-limited", or "the network is down". The CLI's own state filter (e.g. ``--state merged`` for ``gh``, or ``--state tag("state", "merged")`` for tag-driven filtering) is what produces the cleanable-vs-not discrimination; users who need a different shape (e.g. ``tea`` which has no native ``--state merged``) compose their own pipeline with ``jq`` and emit exit code from that. Tags passed via ``gww clean --tag state=closed`` allow the same provider to filter different branch states. See ADR-0018.

**`gww clean` `--tag`**:
Repeatable `key=value` flag on `gww clean`, mirroring `clone` / `add` / `remove`. Feeds `tag()` and `tag_exist()` in both the provider's `when` predicate (for provider selection) and the provider's `filter` template (for per-branch filtering logic). Absent tags default to empty string unless the template uses `tag("name", "default")` to provide a fallback. Common use case: `gww clean --tag state=closed` to remove branches with closed-but-not-merged PRs, where the provider's `filter` uses `--state tag("state", "merged")` to check different states based on the tag.
