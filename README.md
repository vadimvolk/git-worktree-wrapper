...
Each action rule also accepts an optional `critical:` flag (default `true`). When `true`, a failing action in that rule aborts the remaining actions in the rule and makes the command exit `1`. Set `critical: false` for best-effort rules — failures are reported in the action execution summary but the command still exits `0`. See [Failure handling](#-failure-handling) below for the full exit-code table.
```yml
actions:
  - when: file_exists("package.json")
    critical: false  # best-effort: a missing node_modules is annoying, not fatal
    after_clone:
      - command: "npm install"
```
